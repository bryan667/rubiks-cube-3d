...
const CornerDimensions = (size: number, face: string, corner: string, offset?: number): CornerPoints =>{

    (Math as any).csc = function(x: number) { return 1 / Math.sin(x); }
    ...
    let c = (Math as any).csc(dToR(36))*halfPoint;
    ...
    const faces: Record<string, Record<string, CornerPoints>> = {
        face1 : {
            corner1 : {
                p1 : [0, size*ratio],
                p2 : [x1,y1],
