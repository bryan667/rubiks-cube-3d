...
export interface DecaPiece extends THREE.Mesh {
...
export type DecaObject = Record<string, DecaFace>;
...
let swapColors = (face: string, deca: DecaObject, speed: number): void => {

    // Pulls the hex color value from piece
    let hex = (piece: DecaPiece): number => piece.material.color.getHex();

    // Copies b's color to a
    let setColor = (a: DecaPiece, b: DecaPiece) => { a.material.color.set(hex(b)); };
    ...
    let extractEdge = (sideColor: string, pieces: number[]): EdgeGroup => {
        return {
    ...
    let swap = (e1: EdgeGroup, e2: EdgeGroup, e3: EdgeGroup, e4: EdgeGroup, e5: EdgeGroup, speed: number): void => {
    ...
    let edge1: EdgeGroup, edge2: EdgeGroup, edge3: EdgeGroup, edge4: EdgeGroup, edge5: EdgeGroup;
